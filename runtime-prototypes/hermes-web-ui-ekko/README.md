...
| 脚本 | 说明 |
|---|---|
| `install.bat` | 一键安装 + 配置 Hermes 地址 + 启动（新机首选） |
| `config.bat` | 可视化配置 Hermes 连接地址（自动扫描端口 + 点击选择） |
...
├── .env                 # 本地运行时配置（config.bat 自动生成，不入库）
├── .gitignore
├── install.bat          # 一键安装+配置+启动
├── config.bat           # 可视化配置 Hermes 地址
├── start.bat            # 启动脚本
├── stop.bat             # 停止脚本
├── status.bat           # 状态脚本
└── update.bat           # 更新脚本
...
| 变量 | 默认值 | 说明 |
|---|---|---|
| `UPSTREAM` | `http://127.0.0.1:8642` | Hermes Agent 地址，`config.bat` 自动写入 `.env` |
...
通过 `config.bat` 配置 `UPSTREAM` 环境变量指向 Hermes Agent 地址。本机 Hermes Agent 通常在 `127.0.0.1:8056` 运行。
...
- 如无需认证，`start.bat` 默认设置 `AUTH_DISABLED=1`
- 局域网访问默认开启，启动日志中会显示 LAN 地址如 `http://192.168.x.x:8648`
- 迁移机器时只需重装 Node.js + `npm i -g hermes-web-ui`，再运行 `config.bat` 重新配置地址即可
