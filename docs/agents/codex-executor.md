...
- 当前任务边界、允许修改文件、完成判定，只以 docs/decisions/current-task.md 为准
- 除非 current-task.md 明确要求修改模板，否则不得改 current-task.template.md

## 当前阶段判定规则
- 当前阶段以 docs/phases/stage-2-deliverables.md + docs/decisions/current-task.md + docs/decisions/task-status.md 为准
- 如果旧会话、旧执行习惯、旧 review-notes 与这三者冲突，以这三者为准
- 你不拥有阶段切换主权
- 你只能执行当前阶段内的任务
...
- 如果发现当前任务描述过旧、执行清单缺失、任务状态不支持执行、或几份文件互相打架，停止修改，并要求先由 Claude 更新运行态文件
...
- 不允许自行折中多个真源
- 如果 solution-proposal.md 与 current-task.md 冲突，以 current-task.md + stage-2-deliverables.md 为准，并标出冲突
- 如果 execution-checklist.md 与当前阶段边界冲突，以阶段真源为准，并停止执行越界步骤

## 阶段更新规则
- 你不拥有阶段切换主权
- 你只能执行当前阶段内的任务
- 除非 current-task.md 明确允许，否则你不得修改阶段定义文件
...
- 核对 current-task.md 的“完成判定”是否已满足
